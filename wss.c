...
typedef struct{
  ...
  uint16_t  WgtCharHdle;                  /**< WGT Characteristic Handle */
...
/* Private variables ---------------------------------------------------------*/
static const uint16_t SizeWgt = 1;
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch(p_blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
        {
          ...
          if(p_attribute_modified->Attr_Handle == (WSS_Context.WgtCharHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch(p_attribute_modified->Attr_Data[0])
            {
              ...
              case (0x00):
                ...
                notification.EvtOpcode = WSS_WGT_NOTIFY_DISABLED_EVT;
              ...
              case GATT_CHAR_UPDATE_SEND_NOTIFICATION:
                ...
                notification.EvtOpcode = WSS_WGT_NOTIFY_ENABLED_EVT;
                WSS_Notification(&notification);
...
void WSS_Init(void)
{
   ...
   *                                2 for WGT +
   *                                1 for WGT configuration descriptor +
   *                              = 4
  ...
  {
    LOG_INFO_APP("  Success: aci_gatt_add_service command: WssSvcHdle = 0x%04X\n",WSS_Context.WssSvcHdle);
  }

  /**
   * WGT
   */
  uuid.Char_UUID_16 = 0x30fb;
  ret = aci_gatt_add_char(WSS_Context.WssSvcHdle,
                          ...
                          &(WSS_Context.WgtCharHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("  Fail   : aci_gatt_add_char command   : WGT, error code: 0x%2X\n", ret);
  }
  else
  {
    LOG_INFO_APP("  Success: aci_gatt_add_char command   : WgtCharHdle = 0x%04X\n",WSS_Context.WgtCharHdle);
  }
...
tBleStatus WSS_UpdateValue(WSS_CharOpcode_t CharOpcode, WSS_Data_t *pData)
{
  ...
  switch(CharOpcode)
  {
    case WSS_WGT:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        LOG_DEBUG_APP("  Fail   : aci_gatt_update_char_value WGT command, error code: 0x%2X\n", ret);
      }
      else
      {
        LOG_DEBUG_APP("  Success: aci_gatt_update_char_value WGT command\n");
      }
